%{
usage: MultVarRegGASub_DRV

This will perform multiple simulations/replications of the misspecified
multivariate regression with the GA and information criteria script
MultVarRegGASub.
...
if isempty(mult_run)
	...
	data_type = 1+isequal('Real',questdlg('Use Simulated or Real Data?','MultVarRegGA','Simulated','Real','Simulated')); 
	...
	RFs = CovSmooth; ICs = MultVarRegGASub_IC;
	...
	answer = inputdlg(prompt,'MultVarRegGA Parameters',1,def,'off');
	...
	constraint_check = 0;	% JAH 20140330 this used for multiple regression, but not yet MV regression
...
if isempty(mult_run)
    ...
    MRscores = []; MRfil = []; MRchroms = []; mrcnt = 1; pretottim = 0;
    mult_run = [sprintf(['MVR_',df,'_%4.0f%02.0f%02.0f_%02.0f%02.0f%02.0f.mat'],nw)];
...
while mrcnt <= num_reps
    ...
    % save the stuff
    load([mydir,mult_run])
    MRscores = [MRscores;[mrcnt,GA_BEST(1,1),Inf,ga_toc]];	% JAH 20140328 added Inf place holder for constraints violated (used with MultRegGASub_DRV)
    MRfil = [MRfil;StrPad(save_prefix([(length(mydir)+1):length(save_prefix)]),100,'R',' ')];
    MRchroms = [MRchroms;[mrcnt,GA_BEST(1,chromloc)]];
    clear GA_BEST save_prefix chromloc ga_toc
...
%{
JAH 20070403, adapted for octave 3.4.3 20120219
