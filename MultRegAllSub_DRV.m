%{
usage: MultRegAllSub_DRV

Perform all subsets analysis for multiple regression (only the regressors are subset),
and compute various information criteria scores.  If you tell this to use simulated
data, this can run multiple simulations, and determine how often each criterion
selected the "true" model.
...
data_type = 1+isequal('Real',questdlg('Use Simulated or Real Data?',...
    'MultRegAllSub_DRV','Simulated','Real','Simulated'));    % 1=sim, 2 = real

% JAH comment out regul_func stuff 20140330 because not used
if data_type == 1   % simulated
    ...
    answer = inputdlg({'Number Simulations','Number Noise Variables',...
        'Number Observations','Random State'},'Experiment Parameters',...
        1,{'100','6','500','0'},'off');
    % get the parameters
    ...
    if char(answer(4)) == '0';
		...
		rnd_stat = str2num(char(answer(4)));
    end
...
KeepConstant = 0;	% JAH 20140330 added; this is only used for the GA
dispgran = ceil(0.2*mciter);

	% JAH added constraint checking inputs 20131214
	prompt = {'Hypo Tests alpha','VIF Limit','Heteroskedasticity Test (-BPK,-W,-Ws)',...
		'Model alpha Weight','Multicollinearity Weight','Error Nonnormality Weight','Error Heteroskedasticity Weight'};
	def = {'0.05','7','-BPK','0.25','0.25','0.25','0.25'};
	answer = inputdlg(prompt,'Constraint Parameters',1,def,'off');
	constlim=[];
	constlim.alpha = str2double(answer{1});
	constlim.VIFbound = str2double(answer{2});
	constlim.HeteroTest = answer{3};
	% get the constraint weights and normalize added JAH 20150630
	tmp = str2double(answer(4:end));
	tmp(tmp < 0) = 0;
	tmp = tmp / sum(tmp);
...
IC = MultRegGASub_IC; numIC = length(IC);
...
% initialize storage tables JAH 20140330 subset_scores will hold scores + total constraints violated
subset_scores = ones(numsubs,numIC+1)*Inf;
modpicks = zeros(numsubs,numIC);
equl_truemod = zeros(numIC,1);
incl_truemod = equl_truemod;
subset_consts = cell(numsubs,1);	% JAH 20140330 will hold structs with all constraint test results

% output JAH changed 20120309 in octave 'output' has to go on mydir
...
% display settings
disp(repmat('@',1,50))
disp(sprintf('MultRegAllSub_DRV run on %4.0f%02.0f%02.0f_%02.0f%02.0f%02.0f',clock))
disp(repmat('#',1,50))
if data_type == 1
...
for mccnt = 1:mciter    
    ...
    for subcnt = 1:numsubs
		% JAH edited to take in the limits for the contraint testing 20140330 and also to only do regression once
		[subset_scores(subcnt,[1:(end-1)]),lof,pen,bet,tmp] = MultRegGASub_IC('ALL',Y,X,subsets(subcnt,:),'MLE',constlim);
        subset_consts(subcnt) = {tmp}; % JAH 20150630 can't put a struct directly into a cell without the {} in MATLAB, though it worked ok in octave
		subset_scores(subcnt,end) = subset_consts{subcnt}.SCORE;
    end             % subsets loop
...
% prepare row headers
% JAH 20140330 add building the column of constraint violators
constword = repmat(' ',numsubs,length(subset_consts{1}.STR));
rwhds = cell(numsubs,1);
for rcnt = 1:numsubs
    vrs = find(vars.*subsets(rcnt,:)) - HasConstant;
    vrs = sprintf('%d,',vrs);
    rwhds{rcnt} = ['{',vrs([1:(end-1)]),'}'];
    constword(rcnt,:) = subset_consts{rcnt}.STR;
end                 % replications loop
tab = table2str([IC;'Const. Viola.'],subset_scores,{'%0.2f'},0,rwhds);
tab = [tab,[repmat(['-';' ';'-'],1,size(constword,2));constword;repmat('-',1,size(constword,2))]];
...
clear stats vars rwhds numIC tab* currgrp grps mydir tmp lof pen bet

disp(sprintf('\nMultRegAllSub_DRV Required \n\t%1.4f Seconds\n\t%1.4f Minutes\n\t%1.4f Hours',etime(clock,drvstt)./[1,60,3600]));
...
Copyright (C) 2009 J. Andrew Howe
