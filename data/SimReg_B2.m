function [y,x,trueM] = SimReg_B2(n,numred,mv)
%{ [responses, predictors, true model] = SimReg_B2(number observations, number junk variables,multivariate flag)
  ...
  using beta = 2.  The true model is [x1,x2,x3,x4].
  ...
  [y,x,trueM] = SimReg_B2(100,15,1);

  See Also SimReg_B075, SimReg_B1, SimReg_B15, SimReg_Mixt, SimReg_Skew
...
if not(isscalar(n)) || (nargin < 2) || (nargin > 3) || not(isscalar(numred))
    ...
    fprintf('SimReg_B2: INVALID USAGE-Please read the following instructions!\n'), help SimReg_B2, return
end
