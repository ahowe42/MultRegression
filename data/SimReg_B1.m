function [y,x,trueM] = SimReg_B1(n,numred,mv)
%{ [responses, predictors, true model] = SimReg_B1(number observations,number redundant variables,multivariate flag)
  ...
  using beta = 1 (hence, Gaussian).  The true model is [x1,x2,x3,x4].
...
if not(isscalar(n)) || (nargin < 2) || (nargin > 3) || not(isscalar(numred))
    ...
    fprintf('SimReg_B1: INVALID USAGE-Please read the following instructions!\n'), help SimReg_B1, return
end
