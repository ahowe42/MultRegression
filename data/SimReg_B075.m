...
%{ [responses, predictors, true model] = SimReg_B075(number observations, number junk variables,multivariate flag)
  ...
  using beta = 0.75.  The true model is [x1,x2,x3,x4].
  ...
  [y,x,trueM] = SimReg_B075(100,15,1);

  See Also SimReg_B1, SimReg_B15, SimReg_B2, SimReg_Mixt, SimReg_Skew
...
if not(isscalar(n)) || (nargin < 2) || (nargin > 3) || not(isscalar(numred))
    ...
    fprintf('SimReg_B075: INVALID USAGE-Please read the following instructions!\n'), help SimReg_B075, return
end
